...
static const char scancode_to_ascii[128] = {
    ...
    0,   'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';', '\'', '`',
    0,   '\\', 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/',
    0, '*', 0, ' ', 0,  // Right shift, Keypad *, Left Alt, Space, Caps Lock
...
void ps2_set_leds(bool scroll, bool num, bool caps) {
    uint8_t led_byte = 0;
    if (scroll) led_byte |= PS2_LED_SCROLL_LOCK;
    if (num) led_byte |= PS2_LED_NUM_LOCK;
    if (caps) led_byte |= PS2_LED_CAPS_LOCK;
...
void ps2_keyboard_handler(registers_t *regs) {
    ...
    if (!key_released) {
        if (scancode == SC_CAPS_LOCK) {
            ...
            ps2_set_leds(modifiers.scroll_lock, modifiers.num_lock, modifiers.caps_lock);
            extended = false;
            irq_sendEOI(1);
            return;
        }
    ...
    if ((modifiers.ctrl_left || modifiers.ctrl_right) && ascii >= 'a' && ascii <= 'z') {
        ascii = ascii - 'a' + 1;
    } else if ((modifiers.ctrl_left || modifiers.ctrl_right) && ascii >= 'A' && ascii <= 'Z') {
        ascii = ascii - 'A' + 1;
    }
